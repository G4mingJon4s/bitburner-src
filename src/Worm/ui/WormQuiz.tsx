...
export function WormQuiz({ questions }: IProps) {
	...
	return (
		<div>
			<VisualElement />	
			{isDone && <Typography>{guess === currentQuestion.answer ? "Correct!" : "Wrong."} {currentQuestion.explanation}</Typography>}
			{!isDone && <Typography>{currentQuestion.text}</Typography>}
			{!isDone && currentQuestion.type === "text" && <Input onChange={event => setGuess(event.target.value)} placeholder="ABCD"/>}
			{!isDone && currentQuestion.type === "number" && <NumberInput onChange={n => setGuess(n.toString())} placeholder="1234"/>}
			{!isDone && currentQuestion.type === "truthy" && <Switch onChange={event => setGuess(event.target.value ? "true" : "false")}/>}
			...
			{isDone && <Button onClick={() => {
				...
				setIsDone(false);
			}}>{stage + 1 === questions.length ? "Reset" : "Continue"}</Button>}
